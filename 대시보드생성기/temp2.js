...
(function () {
  ...
  function aggregateByBranch(records) {
    ...
    records.forEach(function (r) {
      if (!map[r[0]]) map[r[0]] = emptyBucket();
      addToBucket(map[r[0]], r);
    });
    return Object.keys(map).map(function (b) {
      return { branch: b, bucket: map[b], count: bucketTotal(map[b], "count"), fee: bucketTotal(map[b], "fee") };
    });
  }
  ...
  function renderKPIs(records) {
    ...
    var zScores = window.calculateZScores(cancelRates);
    var anomalies = [];
  ...
  function computeProjection(records, months, actualVals) {
    ...
    var reg = (projMonths.length && xsFit.length >= 2) ? linearRegression(xsFit, fitVals) : null;
    var projVals = reg ? projMonths.map(function (ym) { return Math.max(0, reg.predict(Number(ym.slice(5, 7)))); }) : [];
  ...
  function renderMiniLineChart(prefix, months, actualVals, colorVar, seriesKey, proj, metric, tooltipRowsFn) {
    ...
    if (hasProjection) {
      var line1 = "점선 구간(" + trendBucketLabel(projMonths[0], "month", monthMultiYear) + "~" + trendBucketLabel(projMonths[projMonths.length - 1], "month", monthMultiYear) + ")은 완료된 " + completeCount + "개월의 선형회귀 추세를 연장한 예측치입니다.";
